...
/**
 * Created by momon on 2017/08/18.
 */
...
public class GameScreen extends ScreenAdapter {
    ...
    public GameScreen(JumpActionGame game) {
        ...
        mPrefs = Gdx.app.getPreferences("jp.techacademy.kawai.momoko.jumpactiongame"); // ←追加する
    ...
    public void render (float delta) {
        ...
        if (mGameState == GAME_STATE_GAMEOVER){
            mFont.draw(mGame.batch, "GAME OVER", GUI_WIDTH / 2 - 50, GUI_HEIGHT / 2);
        }
        else if (mGameState == GAME_STATE_CLEAR){
            mFont.draw(mGame.batch, "CLEAR!!!", GUI_WIDTH / 2 - 50, GUI_HEIGHT / 2);
        }
        ...
        for (int i = 0; i < mEnemys.size(); i++) {
            ...
            if (mPlayer.getBoundingRectangle().overlaps(enemy.getBoundingRectangle())) {
                ...
                if (mGame.mRequestHandler != null) {
                    mGame.mRequestHandler.soundEffect();
                }
                return;
            }
        }
